...
import { Apartment as HotelIcon, ExpandMore as ExpandIcon } from '@material-ui/icons'
...
const Hotels = ({ cityCode, checkInDate, checkOutDate, setOfferId }) => {
  ...
  useEffect(() => {
    if (cityCode) {
      setLoading(true)
      getHotels(cityCode, checkInDate.format('YYYY-MM-DD'), checkOutDate.format('YYYY-MM-DD'))
        .then((hotels) => {
          setHotels(hotels)
          setLoading(false)
        })
    ...
    <div className={classes.hotelList}>
      {hotels &&
        hotels.map((hotel) => {
          ...
          return (
            <Accordion key={hotelId} expanded={active} onChange={handleChange(hotelId)}>
              <AccordionSummary expandIcon={<ExpandIcon />}>
                <div className={classes.hotelListing}>
                  <div className={classes.hotelGraphic}>
                    {image ? (
                      <img src={image} alt='HOTEL' className={classes.hotelImage} />
                    ) : (
                      <HotelIcon className={classes.hotelIcon} />
                    )}
                  </div>
                  <div className={classes.hotelDetails}>
                    <Typography className={classes.hotelName}>{name}</Typography>
                    <Typography color='textSecondary' className={classes.hotelAddress}>
